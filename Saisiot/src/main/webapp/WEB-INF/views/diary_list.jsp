...
<script>
	...
	function list(page) {
		location.href = "${path}/mvc03/diarylist.do?curPage=" + page
...
<body>
	...
	<form name="form1" method="post" action="${path}/mvc03/listall.do">
			...
			<c:otherwise>
				<c:forEach var="row" items="${map.list}">
					<c:if test="${row.groupsq eq 0}">
						...
						<!-- 댓글리스트 -->
						<c:forEach var="cmt" items="${map.list}">
							<c:if test="${cmt.groupsq > 0}" >
